...
import { X, Database, FileSpreadsheet, Trash2, Search, CheckCircle2, AlertTriangle, Key, Cloud, RefreshCw, ExternalLink, Mail, FolderOpen, Settings2, Link2, Send } from 'lucide-react';
...
export default function LeadConsoleModal({ isOpen, onClose }: LeadConsoleModalProps) {
  ...
  const [testEmailResult, setTestEmailResult] = useState<{ status: 'success' | 'error'; message: string; details?: any } | null>(null);

  const [newPasskey, setNewPasskey] = useState('');
  const [isUpdatingPasskey, setIsUpdatingPasskey] = useState(false);

  // Listen for OAuth Success postMessage signals from popups
  useEffect(() => {
    const handleOAuthSuccessMessage = (event: MessageEvent) => {
      const origin = event.origin;
      if (!origin.endsWith('.run.app') && !origin.includes('localhost') && !origin.includes('127.0.0.1')) {
        return;
      }
      if (event.data?.type === 'OAUTH_AUTH_SUCCESS') {
        console.log("[OAUTH POPUP] Authorized successfully.");
        fetchWorkspaceConfig();
      }
    };
    window.addEventListener('message', handleOAuthSuccessMessage);
    return () => window.removeEventListener('message', handleOAuthSuccessMessage);
  }, []);

  const handleCustomGoogleOAuth = async () => {
    try {
      ...
      if (!saveRes.ok) {
        throw new Error("Failed to pre-save Google client credentials on server.");
      }

      const authUrl = `/api/workspace/google-oauth-start?clientId=${encodeURIComponent(cleanId)}&clientSecret=${encodeURIComponent(cleanSecret)}`;
      const authWindow = window.open(authUrl, 'google_oauth_popup', 'width=600,height=700');
      if (!authWindow) {
        alert("Please allow popups for this site to link your Google account permanently.");
      }
    } catch (err: any) {
      console.error("Permanent connection start failed:", err);
      alert(err.message || "Could not launch OAuth permission screen.");
    } finally {
  ...
  const handleUpdatePasskey = async (e: React.FormEvent) => {
    ...
    if (cleanPass.length < 5) {
      alert("For better security, the passphrase should be at least 5 characters long!");
      return;
    }
    try {
      ...
      const res = await fetch('/api/workspace/save-config', {
        ...
        body: JSON.stringify({ consolePasskey: cleanPass })
      });
      const data = await res.json();
      if (res.ok && data.status === 'success') {
        alert(`Success!\n\nThe Admin Lead Console passphrase has been successfully updated to:\n"${cleanPass}"\n\nPlease use this new passphrase the next time you log in.`);
        setNewPasskey('');
      } else {
        alert(`Error updating passphrase: ${data.message || "Unknown server error"}`);
      }
    } catch (err: any) {
      console.error(err);
  ...
  const handleSendTestEmail = async () => {
    try {
      ...
      const res = await fetch('/api/workspace/test-email', {
        method: 'POST'
      });
      const data = await res.json();
      if (res.ok && data.status === 'success') {
        setTestEmailResult({ status: 'success', message: data.message });
        alert("Success! Connection Verified!\n\n" + data.message);
      } else {
        const errorMsg = data.message || "Failed to dispatch test email.";
        const errorDetails = data.details ? JSON.stringify(data.details, null, 2) : "Unknown Google Workspace error.";
        setTestEmailResult({ status: 'error', message: errorMsg, details: errorDetails });
        console.error("Test email failure details:", data);
        alert(`Failed to send test email!\n\nReason: ${errorMsg}\n\nTechnical details from Google APIs:\n${errorDetails}\n\nPlease click Disconnect, link your account again and double-check you marked all the Google login permissions checkboxes.`);
      }
    } catch (err: any) {
      console.error("Error launching test email:", err);
      alert(`Network Error: ${err.message}`);
    } finally {
  ...
  // Secure server-side check with fallback authorization
  const handleUnlock = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!password) {
      setErrorText("Please input a passphrase.");
      return;
    }
    try {
      ...
      if (res.ok && data.authenticated) {
        ...
        setErrorText(data.message || 'Access Denied. Please input the correct custom passphrase or fallback recovery passkey.');
      }
    } catch (err: any) {
      console.error(err);
          ...
          <div className="p-10 flex flex-col items-center justify-center space-y-6 text-center max-w-md mx-auto my-auto overflow-y-auto">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1.5 leading-relaxed">
                For security reasons, view of live CRM leads is restricted. Please enter your administrator master access passkey or your custom configured console password.
              </p>
            </div>
              ...
              {activeTab === 'submissions' ? (
                ...
                <div className="space-y-6 max-w-3xl mx-auto py-4">
                  ...
                  {workspaceConfig?.isConnected ? (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-xs space-y-4">
                          ...
                          <div className="bg-slate-50 p-3.5 rounded-lg border border-slate-100 space-y-2.5">
                            ...
                            {workspaceConfig.gmailAlertsActive && (
                              <button
                                onClick={handleSendTestEmail}
                                disabled={isSendingTestEmail}
                                className="w-full mt-2 py-2 bg-red-50 hover:bg-red-100 disabled:bg-slate-100 text-red-700 disabled:text-slate-400 border border-red-200 disabled:border-slate-200 font-extrabold text-xs rounded-lg flex items-center justify-center gap-1.5 transition-colors cursor-pointer select-none"
                                title="Click to transmit a live connection self-test email"
                              >
                                {isSendingTestEmail ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Send className="w-3.5 h-3.5" />}
                                <span>Send Test Connection Email</span>
                              </button>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-xs space-y-4">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-xs font-extrabold text-slate-800">Console Security Passphrase</span>
                            </div>
                          </div>

                          <form onSubmit={handleUpdatePasskey} className="bg-slate-50 p-3.5 rounded-lg border border-slate-100 space-y-3">
                            <div className="text-xs text-slate-600 leading-normal">
                              Set a custom passkey to lock this administrative console. (The secure fallback is configured as "SM@ShivMadh@SM" for emergency recovery).
                            </div>
                            <div className="relative">
                              <input
                                type="text"
                                placeholder="Enter custom passkey (e.g. MyPaperPass123)"
                                value={newPasskey}
                                onChange={(e) => setNewPasskey(e.target.value)}
                                className="w-full text-xs bg-white border border-slate-300 rounded-lg px-3 py-2 outline-none font-bold placeholder-slate-400 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all font-mono"
                              />
                            </div>
                            <button
                              type="submit"
                              disabled={isUpdatingPasskey}
                              className="w-full py-2 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 text-white font-extrabold text-xs rounded-lg flex items-center justify-center gap-1.5 transition-colors cursor-pointer select-none"
                            >
                              {isUpdatingPasskey ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Key className="w-3.5 h-3.5" />}
                              <span>Update Console Passkey</span>
                            </button>
                          </form>
                        </div>
                      </div>
                    ...
                    /* Setup instructions and Link Button with Dual Connection modes */
                    <div className="space-y-6">
                      <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-xs space-y-4">
                        ...
                        <div className="space-y-1">
                          <h4 className="text-sm font-extrabold text-slate-900">Authorize Google Workspace Integration</h4>
                          <p className="text-xs text-slate-500 leading-relaxed">
                            Link your Google Account to activate Google Sheets RFQ logging and real-time Gmail sourcing alerts. Choose between a quick-trial 1-hour session or a permanent, robust background connection.
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-4 flex flex-col justify-between">
                          <div className="space-y-2">
                            <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-extrabold bg-slate-100 text-slate-700 uppercase tracking-wide">
                              Option A: Quick Setup
                            </span>
                            <h5 className="text-xs font-black text-slate-900">Standard Google Sign-In</h5>
                            <p className="text-[11px] text-slate-500 leading-relaxed">
                              Sign in instantly using a 1-click Google popup window.
                            </p>
                            <div className="p-2.5 bg-amber-50/60 border border-amber-100 rounded-lg text-[10.5px] text-amber-800 leading-normal">
                              ⚠️ <strong>Limitation:</strong> Google restricts client-side tokens to **1 hour maximum token life**. You will need to click reconnect every hour to maintain sheets syncing.
                            </div>
                          </div>
                          ...
                          <button
                            ...
                            disabled={isConfiguring}
                            className="w-full py-2.5 bg-slate-800 hover:bg-slate-900 text-white font-bold text-xs rounded-lg flex items-center justify-center gap-1.5 transition-colors cursor-pointer select-none disabled:bg-slate-400 mt-2"
                          >
                            {isConfiguring ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Cloud className="w-3.5 h-3.5" />}
                            <span>Quick Link (1-Hour Session)</span>
                          </button>
                        </div>

                        {/* Option 2: Permanent oAuth */}
                        <div className="bg-white border border-[#7D0909]/20 rounded-xl p-5 space-y-4 flex flex-col justify-between shadow-xs">
                          <div className="space-y-2">
                            <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-extrabold bg-red-50 text-[#7D0909] uppercase tracking-wide border border-rose-100">
                              Option B: Permanent Connection (Best for Teams)
                            </span>
                            <h5 className="text-xs font-black text-slate-900">Custom Google Cloud Console Linking</h5>
                            <p className="text-[11px] text-slate-500 leading-relaxed">
                              Configure with your own Google Cloud Console client credentials to grant the server continuous backup permissions that **never expire**.
                            </p>
                            
                            <div className="bg-slate-50 p-3 rounded-lg border border-slate-100 space-y-2 text-[10px] text-slate-600 leading-normal">
                              <p className="font-extrabold text-[#7D0909]">📋 How to set up in 2 minutes:</p>
                              <ol className="list-decimal list-inside space-y-1">
                                <li>Open <a href="https://console.cloud.google.com/apis/credentials" target="_blank" rel="noopener noreferrer" className="text-indigo-600 underline font-bold">Google Cloud Credentials</a>.</li>
                                <li>Create an <strong>OAuth 2.0 Client ID</strong> (Web Application).</li>
                                <li>Add this URL as an <strong>Authorized Redirect URI</strong>:
                                  <code className="block bg-zinc-200 text-slate-800 font-mono text-[9px] px-1.5 py-1 rounded select-all font-bold mt-1 tracking-tight break-all border border-slate-300">
                                    {window.location.origin}/api/workspace/google-oauth-callback
                                  </code>
                                </li>
                                <li>Paste the generated parameters down below:</li>
                              </ol>
                            </div>

                            <div className="space-y-2.5 pt-1">
                              <div>
                                <label className="block text-[10px] font-extrabold text-slate-700 mb-1 uppercase">Google Client ID</label>
                                <input
                                  type="text"
                                  placeholder="123456-example.apps.googleusercontent.com"
                                  value={clientId}
                                  onChange={(e) => setClientId(e.target.value)}
                                  className="w-full text-[11px] bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5 outline-none font-medium focus:bg-white focus:border-[#7D0909] transition-colors"
                                />
                              </div>
                              <div>
                                <label className="block text-[10px] font-extrabold text-slate-700 mb-1 uppercase">Google Client Secret</label>
                                <input
                                  type="password"
                                  placeholder="GOCSPX-example_secret_key"
                                  value={clientSecret}
                          ...
                          <button
                            onClick={handleCustomGoogleOAuth}
                            disabled={isConfiguring || !clientId.trim() || !clientSecret.trim()}
                            className="w-full py-2.5 bg-[#7D0909] hover:bg-[#991111] disabled:bg-slate-300 disabled:text-slate-500 text-white font-extrabold text-xs rounded-lg flex items-center justify-center gap-1.5 transition-colors cursor-pointer select-none mt-4"
                          >
                            {isConfiguring ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Link2 className="w-3.5 h-3.5" />}
                            <span>Link Permanently (Never Expires)</span>
                          </button>
                        </div>
                      </div>

                      <div className="text-[10px] text-slate-400 leading-normal text-center">
                        All configuration keys are managed with strict enterprise-grade privacy protection directly in your dedicated environment container.
                      </div>
                    </div>
                  )}
                </div>
