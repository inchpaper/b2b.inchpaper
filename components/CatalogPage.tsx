...
import { formatCatalogImageUrl, getGoogleDriveFallbackUrl } from '../utils/imageUrl';
...
export default function CatalogPage({ 
  ...
  const [isAdminModalOpen, setIsAdminModalOpen] = useState(() => {
    if (typeof window !== 'undefined') {
      ...
      return search.includes('admin') || search.includes('manage') || hash.includes('admin') || hash.includes('manage');
    }
    return false;
  });
  ...
  useEffect(() => {
    const handleUrlCheck = () => {
      ...
      const search = (window.location.search || '').toLowerCase();
      const hash = (window.location.hash || '').toLowerCase();
      if (search.includes('admin') || search.includes('manage') || hash.includes('admin') || hash.includes('manage')) {
            ...
            {filteredCatalogs.map((catalog) => (
              ...
              >
                ...
                <div className="space-y-4">
                  ...
                  >
                    <img
                      src={formatCatalogImageUrl(catalog.imageUrl)}
                      alt={catalog.title}
                      ...
                      onError={(e) => {
                        ...
                        const fallback = getGoogleDriveFallbackUrl(catalog.imageUrl);
                        if (fallback && target.src !== fallback) {
                          target.src = fallback;
                        } else if (!target.src.includes('unsplash') && !target.src.includes('inchpaper')) {
                          target.src = 'https://images.unsplash.com/photo-1549465220-1a8b9238cd48?w=1200&auto=format&fit=crop&q=80';
                        }
                      }}
                    />
