...
export default function App() {
  ...
  const [currentView, setCurrentView] = useState<'home' | 'catalog'>(() => {
    if (typeof window !== 'undefined') {
      ...
      if (path.includes('catalog') || hash.includes('catalog') || search.includes('catalog')) {
        return 'catalog';
      }
    }
    return 'home';
  ...
  useEffect(() => {
    const checkCurrentRoute = () => {
      ...
      const hash = (window.location.hash || '').toLowerCase();
      const search = (window.location.search || '').toLowerCase();
      const isCatalogRoute = path.includes('catalog') || hash.includes('catalog') || search.includes('catalog');
      setCurrentView(isCatalogRoute ? 'catalog' : 'home');
    };
